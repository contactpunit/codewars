...
export function outed(meet, boss) {
    const happinessScore = Object.entries(meet as { [key: string]: number }).reduce((acc, curr) => {
        if (curr[0] === boss) acc += curr[1] * 2
        else acc += curr[1]
        return acc
    }, 0) / Object.values(meet).length
    return happinessScore > 5 ? 'Nice Work Champ!' : 'Get Out Now!'
}
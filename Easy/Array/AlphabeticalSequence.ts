...
export function alphaSeq (str) {
    const res = str.split('').map(c => c.toLowerCase()).sort((a,b) => a.localeCompare(b))
    return res.map(e => e.toUpperCase().concat(e.repeat(e.charCodeAt(0) - 97))).join(',')
}
...
// decodePass(['password456', 'pass1', 'test12'], '01110000 01100001 01110011 01110011 01110111 01101111 01110010 01100100 00110001 001

export function decodePass(passArr, binNum) {
    const binArr = binNum.split(' ')
    const nums = binArr.map(b => parseInt(b, 2))
    const strs = nums.map(e => String.fromCharCode(e))
    if (passArr.includes(strs.join(''))) return strs.join('')
    return false
}
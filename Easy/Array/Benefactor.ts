...
export function newAvg(arr, newavg) {
    const newLen = arr.length + 1
    const total = arr.reduce((acc, curr) => acc += curr, 0)
    const remain = (newLen * newavg) - total
    if (remain < 0) throw new Error('Expected New Average is too low')
    else {
        return Math.floor(remain) === remain ? remain : Math.floor(remain) + 1
    }
}
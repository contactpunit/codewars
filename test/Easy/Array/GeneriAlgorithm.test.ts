...
import { fitness, mapPopulationFit } from '../../../Easy/Array/GenericAlgorithm'

describe('fitness test suite()', () => {
    const fn = (c) => {
    ...
    it('x %s result is %s', () => {
        const res: any[] = mapPopulationFit(population, fn)
        expect(res[0].fitness).toBe(0.25)
        expect(res[1].fitness).toBe(0.75)
        expect(res.slice(-2, -1)[0].fitness).toBe(0.375)
        expect(res.slice(-1)[0].fitness).toBe(0.875)
    })
})
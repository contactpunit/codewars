...
import { countWins } from '../../../Easy/Array/ChampionsLeague'

describe('countWins() test suite', () => {
  const season = [
    ...
    it.each([
        ...
        [season, 'FootLand', 0],
        [season, 'Spain', 9]
    ])('lamps %s and drone %s returns at %s', (seasons, country, result) => {
        expect(countWins(seasons, country)).toBe(result)
    } )
})